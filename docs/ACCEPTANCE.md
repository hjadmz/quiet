...
Updated 2026-08-27. This report separates retained checks from historical
...
|---|---|---|
| 0 | **A bad parsed setting cannot break quiet's templates, and says so** | `npm run test:hostile` builds fourteen configs — `posts_on_home` at 1000, −10, `abc`, 0, an empty value, a list, and `all`; an every-setting-hostile config carrying CSS-injection, attribute-injection, quoted booleans, an off-site favicon and unusable footer identities; and a valid-but-unusual config that must report nothing. Each asserts the page still has a title, a language, an unrewritten body class, a named home link, the right number of posts, and that every problem is named in words at `/site-check/`. It also covers the values YAML rewrites before any template sees them — `lang: no` (Norwegian, read as `false`), `title: on`, `description: yes`, `author.name: yes` — and `footer:` written flat instead of nested, which parses cleanly and silently drops both footer links. And it builds two posts that resolve to one permalink, a `last_modified_at` Ruby would misread, and a `draft: true`, and asserts each is reported. `npm run doctor` reads that same report and exits non-zero, so local checks fail on a dirty config without restating a single rule. YAML syntax and raw plugin reads are bounded below rather than covered by this claim. |
| 1 | Template, generated-site and configured-demo builds | `npm run verify:template` validates the host-neutral mother template while explicitly allowing its shipped placeholders. `npm run verify` is strict for generated sites. `npm run verify:cloudflare` adds Cloudflare's output headers, and `npm run test:portable` simulates a personalized site with the demo posts and host overlay removed. `npm run verify:demo` checks a configured demo fork's routes, canonical origin, placeholder removal, visible demo/AI disclosure, feed and headers. The build checks cover strict front matter, generated routes, local links/fragments, metadata, output exclusions, deployment headers, and budgets. Redirect-only documents are identified separately from full pages: their canonical, refresh and script targets must agree, they need a fallback link and `noindex`, their target must exist with exact letter case, and they do not inflate the archive's post count. |
| 2 | **The address is a setting, not an assumption** | `npm run test:origins` builds the template at all four shapes a fork can be served from — a github.io user site, a github.io project subfolder, a custom domain root, and a custom domain subfolder — and runs the full generated-site, config and feed checks against each. It replaces a single `/quiet` run, which proved one of the four. Each build is then scanned for any origin it was not given, including the `username.github.io` placeholder the repository ships: a hardcoded host does not announce itself, since the site still builds and still passes its own link checks while sending a fork's readers somewhere else. The scan is held to non-vacuity by counting this build's own absolute URLs in the same files — 84 per shape — so a page set with no absolute URLs cannot pass it by having nothing to find. |
| 2b | **It holds at five hundred posts, and find-in-page still reaches all of them** | `npm run test:scale` generates 500 posts, builds, and asserts what only breaks at size: every post has a page (Jekyll drops one silently rather than failing), the home page still lists at most 20, the feed still carries at most 50 full-content entries, and the sitemap lists all 503 URLs. Measured: **7.8s build, 500 pages, archive 149 KB, home 6 KB, feed 12 KB** — the home page a reader waits for does not grow with the site. The load-bearing assertion is the archive one. Refusing to ship a search box is only defensible while the browser's own find-in-page can reach every post, so all 500 titles **and** all 500 descriptions are checked individually for presence in the archive's text, and the archive is checked for `display: none`, which find-in-page skips. This had been measured once by hand and written down in prose, which is the same as not having checked it: nothing would have failed if a later change made the archive quadratic. |
...
| 4e | **Day one in a generated site leaves no template identity behind** | `npm run test:edge` proves deleting all demo posts or the About page leaves real empty states, a valid zero-entry feed and no dead navigation. It also gives the fork a GitHub Pages `CNAME`, asserts that the file is still published, and prevents `/site-check/` from calling expected custom-domain infrastructure stray output. `npm run test:portable` separately performs the six documented identity edits, rewrites About, deletes the demo posts and Cloudflare overlay, then runs the strict config, generated-site, feed and edge checks. It fails if the output still contains `Your Name`, `username.github.io`, quiet's tagline, or quiet's site description. The mother template alone receives the placeholder exemption; a generated repository does not. The four shipped identity images cannot be identified by Liquid from their pixels, so the README names their replacement explicitly instead of pretending the check can see it. |
| 4f | **The template does not punish or silently excuse someone for using it** | Found by Henry's generated site: an earlier `verify` pinned `SITE_URL=https://username.github.io`, so changing the documented URL made CI fail. That literal origin is gone and `npm run test:portable` prevents its return. The opposite failure was then found: `verify` itself always enabled the template-placeholder exemption, so a generated site could stay green while publishing quiet's identity. The exemption now comes from CI only when `github.repository == 'hjadmz/quiet'`; inherited workflows are strict. `verify:cloudflare` follows the same rule, while its host overlay is gated by file existence rather than repository name. |
...
| 11 | Feed, sitemap, and metadata | The source feed template stays aligned with jekyll-feed 0.17 through a declared transform for each difference, including the validated feed subtitle and two targeted role removals; the plugin still provides discovery metadata and deliberately skips the existing feed path. Strict XML checks cover full entry content, timestamps, authors, canonical/self/footer links, subpaths, footnote targets, and sitemap exclusion. jekyll-sitemap generates the sitemap. `last_modified_at` feeds visible and machine-readable update metadata. |
...
| 17 | Deviations from the vendored feed are declared, not diffed | `tests/feed-smoke.rb` strips explanatory comments, then reconstructs the expected file by applying named transforms to the installed jekyll-feed. An upgrade that removes an anchor fails naming the deviation that stopped applying; a genuine drift fails with the first differing line, ours and expected. The version is read from the installed gem rather than pinned to 0.17.0. |
...
- **`jekyll-seo-tag` reads `site.lang`, `site.author.name`, and
  `site.description` directly.** The first two are printed unescaped; the last
  can still expose a YAML boolean as metadata even though quiet and its feed use
  the validated fallback. All three are reported at `/site-check/`, and
  `tests/config-lint.cjs` fails the
...
The 2026-08-19 audit recorded Lighthouse 100/100/100/100 and zero CLS
for its then-current build. Those numbers were not rerun in later passes and are not
...
2. Ruby is 3.3.12, Bundler is frozen, and the build runs `npm run
   build:cloudflare` after `npm run verify:cloudflare` has passed in CI (a demo
   fork additionally passes `npm run verify:demo`);

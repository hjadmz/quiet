...
Measured 2026-08-18 on the production build (Jekyll 3.10.0 — the same
...
| 7 | Body ≥ 7:1, all text ≥ 4.5:1, both themes | **pass** | computed from OKLCH→sRGB: body 16.4:1 light / 13.7:1 dark; muted 6.7 / 6.9; default accent 5.2 / 7.5; hjadmz accent 7.1 / 7.7; code tokens 6.2–8.7 on their surface |
...
| 13 | Page weight within budget | **pass** | kitchen-sink post: HTML ~13 KB + CSS 13.5 KB + copy.js 1.0 KB ≈ 27.5 KB excluding content images (budget ≤ 60 KB). CSS 13,455 B unminified (≤ 16 KB); theme script 600 B (≤ 600 B); copy.js 1,019 B (≤ 1 KB); total JS 1,619 B (≤ 2 KB) |
...
- **Favicon is the current brand "hj" monogram, not the 16×16 pixel-art
  mark** — the pixel mark was retired in Brand v1.2.0 (canonical wiki);
  resurrecting it would contradict the frozen brand system.

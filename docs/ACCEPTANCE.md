...
| 4e | **Day one in a generated site leaves no template identity behind** | `npm run test:edge` proves deleting all demo posts or the About page leaves real empty states, a valid zero-entry feed and no dead navigation. `npm run test:portable` separately performs the six documented identity edits, rewrites About, deletes the demo posts and Cloudflare overlay, then runs the strict config, generated-site, feed and edge checks. It fails if the output still contains `Your Name`, `username.github.io`, quiet's tagline, or quiet's site description. The mother template alone receives the placeholder exemption; a generated repository does not. The four shipped identity images cannot be identified by Liquid from their pixels, so the README names their replacement explicitly instead of pretending the check can see it. |

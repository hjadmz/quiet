...
## A figure wider than the text

The reading column is 36rem, which is right for a sentence and wrong for a
screenshot. `class="wide"` on a figure opts out of it, up to 52rem, and stops
there so an ultrawide display does not stretch the image across the desk.

<figure class="wide">
  <img src="{{ '/assets/img/demo-figure.png' | relative_url }}"
       alt="The same compositional study, shown wider than the reading column."
       width="1200" height="675" loading="lazy">
  <figcaption>Same figure, given room. The caption keeps the reading measure.</figcaption>
</figure>

## Something that runs

A demo is an ordinary HTML file in `_includes/demos/` — a whole document, with
its own CSS and script, that you can open in a browser and work on normally.
One line in the post puts it here.

The file is inlined at build time, so this page makes no extra request for it,
and it runs in a frame sandboxed without `allow-same-origin`: it cannot read
this page, its cookies, or its storage, and nothing it does can reach the site
around it. With JavaScript off you still see its starting state, which is why a
demo should start somewhere meaningful.

Isolation cuts both ways. A demo cannot see the site's theme toggle either, so
it follows the reader's system setting — write it with `color-scheme: light
dark`, as this one does, and it will look right in both.

{% include demo.html src="demos/hue-mixer.html" title="Hue mixer" height="150" caption="Drag the slider. The swatch follows." %}
